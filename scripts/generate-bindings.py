...
# Name-addressable core classes: (schema name) -> resolver.
...
def main() -> int:
    schema = json.loads((LIB / "docs" / "derived-core-schema.json").read_text())
    by_name = {c["name"]: c for c in schema["classes"]}
    db_h = (LIB / "vendor/OpenROAD/src/odb/include/odb/db.h").read_text()
